...
#define POD_WIDTH        12 //24?
#define POD_HEIGHT       94

#define POD_RIGHT_X      600
#define POD_LEFT_X       6 + POD_WIDTH

#define POD_MOVE_UP      5   //pixels to move up
#define POD_MOVE_DOWN    5   //pixels to move down

int gPodIndex[MAX_PODS];
int gPodDirection[MAX_PODS];
...
int ball_y;
int ball_x;
int ball_x_direction = BALL_SPEED;
int ball_y_direction = BALL_SPEED;

//pong messages / system
float gPodUpdate = 1;
bool GameBegin = FALSE;
bool gHasHitBase;
extern int gmsgPongPod;
extern int gmsgPongBall;

void UpdatePongBall()
{
	int sound;

	//not active..reset/dont move.
	if ( !gPodIndex[LEFT_POD] || !gPodIndex[RIGHT_POD] )
	{
		ball_x = 310;
		ball_y = 200;
		GameBegin = FALSE;
	...
	if ( GameBegin == FALSE )
	{
		gPodUpdate = gpGlobals->time + 6.0;
		GameBegin = TRUE;
		MESSAGE_BEGIN( MSG_ALL, gmsgPongBall );
			WRITE_SHORT(ball_x);
			WRITE_SHORT(ball_y);
			WRITE_BYTE(5);
		MESSAGE_END();
		...
		for ( int i = 1; i <= gpGlobals->maxClients; i++ )
		{
			...
			if ( plr && plr->IsPlayer() )
				ClientPrint(plr->pev, HUD_PRINTCENTER, "Game Begins in 6 Seconds!");
		}
	...
	if ( ball_y >= (480 - BALL_HEIGHT) )
	{
		ball_y_direction = BALL_SPEED * -1;
		sound = 2;
	}
	...
	if ( ball_x >= (640 - BALL_WIDTH) )
	{
		gHasHitBase = TRUE;
		ball_x_direction = BALL_SPEED * -1;
		sound = 4;
	}

	//right pod reflection
	if ( ball_x < POD_RIGHT_X && ball_x > (POD_RIGHT_X - POD_WIDTH) )
	{
		if ( (ball_y > gPodDirection[RIGHT_POD]) && (ball_y < (gPodDirection[RIGHT_POD] + POD_HEIGHT)))
		{	
			ball_x_direction = BALL_SPEED * -1;
			sound = 1;
		}
	...
	//left pod reflection
	if ( ball_x > POD_LEFT_X && ball_x < (POD_LEFT_X + POD_WIDTH) )
	{
		if ( (ball_y > gPodDirection[LEFT_POD]) && (ball_y < (gPodDirection[LEFT_POD] + POD_HEIGHT)))
		{	
			ball_x_direction = BALL_SPEED * 1;
			sound = 1;
		}
	...
	//send ball update to all client huds
	MESSAGE_BEGIN( MSG_ALL, gmsgPongBall );
		WRITE_SHORT(ball_x);
		WRITE_SHORT(ball_y);
		WRITE_BYTE(sound);
...
void CHalfLifeMultiplay :: Think ( void )
{
	...
	if ( gPodUpdate < gpGlobals->time )
	{
		for ( int i = 1; i <= gpGlobals->maxClients; i++ )
		{
			...
			if ( plr && plr->IsPlayer() )
			{
				//see if the selected pod controller is moving up or down
				for ( int j = RIGHT_POD; j <= LEFT_POD; j++ )
				{
					if ( plr->pev->button & IN_FORWARD && gPodIndex[j] == ENTINDEX(ENT(plr->pev)) )
						gPodDirection[j] = gPodDirection[j] - POD_MOVE_UP; 
					else if ( plr->pev->button & IN_BACK && gPodIndex[j] == ENTINDEX(ENT(plr->pev)) )
						gPodDirection[j] = gPodDirection[j] + POD_MOVE_DOWN; 

					//make sure pod doesnt go off screen.
					if ( gPodDirection[j] > ( 480 - (POD_HEIGHT + 6) ))
							gPodDirection[j] = 480 - (POD_HEIGHT + 6);
			    	if ( gPodDirection[j] < 0)
							gPodDirection[j] = 0;
				}
			}
		}

		//send move update to all client huds
		MESSAGE_BEGIN( MSG_ALL, gmsgPongPod );
			WRITE_BYTE(gPodIndex[RIGHT_POD]);
			WRITE_BYTE(gPodIndex[LEFT_POD]);
			for (int p = RIGHT_POD; p <= LEFT_POD; p++ )
				WRITE_SHORT(gPodDirection[p]);
		MESSAGE_END();

		
		gPodUpdate = gpGlobals->time;
		...
		if ( gHasHitBase )
		{
			...
			ball_y = 150;
			ball_x = 300;
			gPodDirection[LEFT_POD] = gPodDirection[RIGHT_POD] = 150;
			gHasHitBase = FALSE;
			return;
		}
	}
...
void CHalfLifeMultiplay :: InitHUD( CBasePlayer *pl )
{
	...
	//start pong
	if ( !gPodIndex[RIGHT_POD] )
	{
		gPodIndex[RIGHT_POD] = ENTINDEX(ENT(pl->pev));
		ClientPrint(pl->pev, HUD_PRINTNOTIFY, "You are the right pod.\n");
	}
	else if ( !gPodIndex[LEFT_POD] )
	{
		gPodIndex[LEFT_POD] = ENTINDEX(ENT(pl->pev));
		ClientPrint(pl->pev, HUD_PRINTNOTIFY, "You are the left pod.\n");
	}
	//end pong
}
