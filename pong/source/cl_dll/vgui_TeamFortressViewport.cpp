...
void TeamFortressViewport::Initialize( void )
{
	...
	if (m_pPongHud)
	{
		m_pPongHud->Initialize();
		ShowPongHud();
		m_pPongHud->setVisible(true);
	}
	//end pong
	if (m_pSpectatorMenu)
	{
...
//start pong
void TeamFortressViewport::ShowPongHud( void )
{
	if (m_pPongHud)
	{	
		m_pPongHud->Open();
...
//start pong
bool TeamFortressViewport::IsPongHudVisible( void )
{
	if (m_pPongHud)
		return m_pPongHud->isVisible();
...
//start pong
void TeamFortressViewport::CreatePongHud( void )
{
	m_pPongHud = new PongHud(0, 0, ScreenWidth, ScreenHeight);
	m_pPongHud->setParent(this);
	m_pPongHud->setVisible(true);
}
//end pong
...
void TeamFortressViewport::paintBackground()
{
	...
	//start pong
	if (m_pPongHud->isVisible() && (m_flPongHudLastUpdated< gHUD.m_flTime) )
	{
		m_pPongHud->Update();
		m_flPongHudLastUpdated = gHUD.m_flTime; // we want this to update quick
	}
	//end pong
...
//start pong
int TeamFortressViewport::MsgFunc_PongPod( const char *pszName, int iSize, void *pbuf )
{
	BEGIN_READ( pbuf, iSize );

	int right_controller = READ_BYTE();
	int left_controller = READ_BYTE();

//	strcpy(m_pPongHud->szRController, g_PlayerInfoList[ right_controller ].name);
//	strcpy(m_pPongHud->szLController, g_PlayerInfoList[ left_controller ].name);

//	if ( !m_pPongHud->szRController )
//		strcpy(m_pPongHud->szRController, "Empty");
//	if ( !m_pPongHud->szLController )
//		strcpy(m_pPongHud->szLController, "Empty");

	m_pPongHud->m_PosRightPod = READ_SHORT();
	m_pPongHud->m_PosLeftPod = READ_SHORT();
	
	//char sz[128];
	//sprintf(sz, "right: %s | left: %s\n", m_pPongHud->szRController, m_pPongHud->szLController );
	//ConsolePrint(sz);

	return 1;
}

void TeamFortressViewport::PlayPongSound ( int sound )
{
	switch ( sound )
	{
	case 1:
		PlaySound("hit_med.wav", 1);
		break;
	case 2:
		PlaySound("hit_high.wav", 1);
		break;
	case 3:
		PlaySound("hit_base.wav", 1);
		m_pPongHud->m_iRightScore += 1;
		break;
	case 4:
		PlaySound("hit_base.wav", 1);
		m_pPongHud->m_iLeftScore += 1;
		break;
	case 5:
		PlaySound("pong_intro.wav", 1);
		break;
	}

}

int TeamFortressViewport::MsgFunc_PongBall( const char *pszName, int iSize, void *pbuf )
{
	BEGIN_READ( pbuf, iSize );

	int sound;

	m_pPongHud->ball_x = READ_SHORT();
	m_pPongHud->ball_y = READ_SHORT();
	sound = READ_BYTE();
