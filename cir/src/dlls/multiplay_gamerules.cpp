...
// override some values for multiplay.
	...
	// Pipe whack
	gSkillData.plrDmgPipe = 25;

	// Deagle
	gSkillData.plrDmgDeagle = 35;

	// Smg Round
	gSkillData.plrDmg9MM = 12;

	//C4 Mine
	gSkillData.plrDmgC4Mine = 100;
...
void CHalfLifeMultiplay :: PlayerSpawn( CBasePlayer *pPlayer )
{
	...
	{
		//start ice
		pPlayer->GiveNamedItem( "weapon_steelpipe" );
		//end ice
	}

	//start ice
	MESSAGE_BEGIN( MSG_BROADCAST, SVC_TEMPENTITY );
		WRITE_BYTE( TE_PARTICLEBURST ); //This denotes which temporary intity we want to use
...
/*
	...
	MESSAGE_END();
*/
	EMIT_SOUND( ENT(pPlayer->pev), CHAN_BODY, "player/pl_spawn1.wav", 1.0, ATTN_NORM );

/*	//start ice
	// Prevent this is the cvar is set
	if ( allow_spectators.value && !pPlayer->m_fIsConnectObserver )
	{
		edict_t *pentSpawnSpot = EntSelectSpawnPoint( pPlayer );
		pPlayer->StartObserver( VARS(pentSpawnSpot)->origin, VARS(pentSpawnSpot)->angles);
		pPlayer->m_fIsConnectObserver = TRUE;
	}
	//end ice
*/
}
