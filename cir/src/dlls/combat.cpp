...
void CBaseMonster :: GibMonster( void )
{
	...
	//start ice
	EMIT_SOUND(ENT(pev), CHAN_WEAPON, "player/pl_bodysplat1.wav", 1, ATTN_NORM);
	//end ice
	...
	while ((pEntity = UTIL_FindEntityInSphere( pEntity, vecSrc, flRadius )) != NULL)
	{
		if ( pEntity->pev->takedamage != DAMAGE_NO )
		{
			...
			{// the explosion can 'see' this entity, so hurt them!
				...
				{
					//ClearMultiDamage( );
					pPlayer->TraceAttack( pevInflictor, flAdjustedDamage, (tr.vecEndPos - vecSrc).Normalize( ), &tr, bitsDamageType );
