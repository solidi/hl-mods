...
void DecalGunshot( TraceResult *pTrace, int iBulletType )
{
	...
	if ( VARS(pTrace->pHit)->solid == SOLID_BSP || VARS(pTrace->pHit)->movetype == MOVETYPE_PUSHSTEP )
	{
		...
		case BULLET_PLAYER_CROWBAR:
			// wall decal
			UTIL_DecalTrace( pTrace, DECAL_PIPESMASH );
...
void W_Precache(void)
{
	...
	// steelpipe
	UTIL_PrecacheOtherWeapon( "weapon_steelpipe" );

	// chainsaw
	UTIL_PrecacheOtherWeapon( "weapon_chainsaw" );

	// pulse gun
	UTIL_PrecacheOtherWeapon( "weapon_pulsegun" );

	// sawedoff
	UTIL_PrecacheOtherWeapon( "weapon_sawedoff" );
	UTIL_PrecacheOther( "ammo_slugbox" );

	// deagle
	UTIL_PrecacheOtherWeapon( "weapon_deagle" );
	UTIL_PrecacheOther( "ammo_deaglebox" );

	// smg
	UTIL_PrecacheOtherWeapon( "weapon_smg" );

	// blaster rifle
	UTIL_PrecacheOtherWeapon( "weapon_blaser" );
...
*/
	...
	//start ice
	PRECACHE_SOUND ("weapons/env_explode1.wav");// explosion 
	PRECACHE_SOUND ("weapons/env_explode2.wav");// explosion 
	PRECACHE_SOUND ("weapons/env_explode3.wav");// explosion 
	PRECACHE_SOUND ("weapons/env_debris1.wav");// explosion aftermaths
	PRECACHE_SOUND ("player/pl_bulletflesh1.wav");
	//end ice
...
{
	//start ice
	if ( m_iId == WEAPON_SMG ) //laser spot update
		WeaponIdle();
	//end ice
...
// to deploy a better weapon that the player may pick up because he has no ammo for it.
	...
	EMIT_SOUND( pOther->edict(), CHAN_ITEM, "player/pl_wpickup1.wav", 1, ATTN_NORM );
	SetTouch(NULL);
	UTIL_Remove(this);
}
...
BOOL CWeaponBox::PackWeapon( CBasePlayerItem *pWeapon )
{
	...
	if ((!strcmp((char *)STRING( pWeapon->pev->classname ), "weapon_steelpipe")))
		SET_MODEL( ENT(pev), "models/w_steelpipe.mdl");
	else if ((!strcmp((char *)STRING( pWeapon->pev->classname ), "weapon_sawedoff")))
	...
	else if ((!strcmp((char *)STRING( pWeapon->pev->classname ), "weapon_smg")))
		SET_MODEL( ENT(pev), "models/w_smg.mdl");
	else if ((!strcmp((char *)STRING( pWeapon->pev->classname ), "weapon_blaser")))
		SET_MODEL( ENT(pev), "models/w_smg.mdl");
	else if ((!strcmp((char *)STRING( pWeapon->pev->classname ), "weapon_c4")))
