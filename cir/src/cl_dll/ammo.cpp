...
void CHudAmmo::Think(void)
{
	...
	if (gHUD.m_iKeyBits & IN_ATTACK)
	{
		...
		//start ice
		gHUD.m_fWeaponSlotIsDown = false;
		//end ice
...
void PlayWeaponSound ( int index )
{
	...
	{
	case 1:
		ConsolePrint("Selected Steel Pipe...\n");
		PlaySound("client/weapon_steelpipe.wav", 1);
		break;
	case 2:
		ConsolePrint("Selected Chainsaw...\n");
		PlaySound("client/weapon_chainsaw.wav", 1);
		break;
	case 3:
		ConsolePrint("Selected PulseGun...\n");
		//PlaySound("client/weapon_pulsegun.wav", 1);
		break;
	case 4:
		ConsolePrint("Selected Desert Eagle...\n");
		PlaySound("client/weapon_deagle.wav", 1);
		break;
	case 5:
		ConsolePrint("Selected Sawed Off Shotgun...\n");
		PlaySound("client/weapon_sawedoff.wav", 1);
		break;
	case 6:
		ConsolePrint("Selected Sub Machine Gun...\n");
		PlaySound("client/weapon_smg.wav", 1);
		break;
	case 7:
		ConsolePrint("Selected Blaser Rifle...\n");
		break;
	case 8:
		ConsolePrint("Selected C4 Mines...\n");
		PlaySound("client/weapon_c4.wav", 1);
		break;
	}
}
//end ice
...
int CHudAmmo::MsgFunc_CurWeapon(const char *pszName, int iSize, void *pbuf )
{
	...
	//start ice
	if ( (m_iCurrentWeapon != pWeapon->iId) )
	{
		if ( gWR.CountAmmo(pWeapon->iAmmoType) || pWeapon->iClip ) 
		{
			PlayWeaponSound ( pWeapon->iId );
			m_iCurrentWeapon = pWeapon->iId;
		}
		else
		{
			//play for -1 ammo (steelpipe and chainsaw)
			if ( pWeapon->iId < 3 && pWeapon->iId > 0 )
			{
				PlayWeaponSound ( pWeapon->iId );
				m_iCurrentWeapon = pWeapon->iId;
			}
...
/*
	...
	pText->setParent( gViewPort );

	// get the font and colors from the scheme
	pText->setFgColor( 0, 115, 230, 0 );
...
/*
	char sz[64];
	sprintf(sz, "iTotalWeapons: %d\n", iTotalWeapons);
	DrawConsoleString(200, 200, sz);
*/
