...
TeamFortressViewport::TeamFortressViewport(int x,int y,int wide,int tall) : Panel(x,y,wide,tall), m_SchemeManager(wide,tall)
{
	...
	SchemeHandle_t hPrimaryScheme = m_SchemeManager.getSchemeHandle( "Primary Button Text" );
	{
		...
		//start ice
		pScheme->setColor(Scheme::sc_primary2, 0, 115, 230, 170 );
		//end ice
		...
		//start ice
		pScheme->setColor(Scheme::sc_secondary1, 0*0.7, 115*0.7, 230*0.7, 0);
		//end ice
	}
...
void TeamFortressViewport::Initialize( void )
{
	...
	if (m_pCirHUD)
	{
		m_pCirHUD->Initialize();
		ShowCirHUD();
		m_pCirHUD->setVisible(true);
	}
	//end ice
...
//start ice
void TeamFortressViewport::ShowCirHUD( void )
{
	if (m_pCirHUD)
	{
		m_pCirHUD->Open();
...
//start ice
bool TeamFortressViewport::IsCirHUDVisible( void )
{
	if (m_pCirHUD)
		return m_pCirHUD->isVisible();
...
//start ice
void TeamFortressViewport::CreateCirHUD( void )
{
	m_pCirHUD = new CirHUD(0,0,ScreenWidth, ScreenHeight);
	m_pCirHUD->setParent(this);
	m_pCirHUD->setVisible(true);
}
//end ice
...
CMenuPanel* TeamFortressViewport::CreateTextWindow( int iTextToShow )
{
	...
	if ( iTextToShow == SHOW_MOTD )
	{
		if (!m_szServerName || !m_szServerName[0])
			//start ice
			strcpy( cTitle, "Cold-Ice Resurrection" );
			//end ice
		else
...
void TeamFortressViewport::paintBackground()
{
	...
	//start ice
	//odd situtation where time is reset after this code is called.
	//this is to ensure the limit hasn't exceeded 0.1 seconds.
	if ( (m_flCirHUDLastUpdated - gHUD.m_flTime) > 0.1 )
		m_flCirHUDLastUpdated = 0;

	if (m_pCirHUD->isVisible() && (m_flCirHUDLastUpdated < gHUD.m_flTime) )
	{
		m_pCirHUD->Update();
		m_flCirHUDLastUpdated = gHUD.m_flTime + 0.1; // we want this to update quick
	}
	//end ice
...
int	TeamFortressViewport::KeyInput( int down, int keynum, const char *pszCurrentBinding )
{
	...
	if (m_iUser1 && gEngfuncs.Con_IsVisible() == false )
	{
		...
		if ( down && (keynum == K_ENTER || keynum == K_KP_ENTER) )
			gEngfuncs.pfnClientCmd( "stop_spectate\n" );
...
int TeamFortressViewport::MsgFunc_ScoreInfo( const char *pszName, int iSize, void *pbuf )
{
	...
	if ( cl > 0 && cl <= MAX_PLAYERS )
	{
		...
		{
			gHUD.m_iMyFrags = frags;
			//gHUD.m_iMyFrags = deaths;
		}
		//end ice
...
//start ice
int TeamFortressViewport::MsgFunc_HealthStat( const char *pszName, int iSize, void *pbuf )
{
	BEGIN_READ( pbuf, iSize );

	for ( int i = 1; i <= 8; i++ )
		gHUD.m_fPlayerHealth[i] = READ_BYTE();

	return 1;
}

int TeamFortressViewport::MsgFunc_Rune( const char *pszName, int iSize, void *pbuf )
{
	BEGIN_READ( pbuf, iSize );

	gHUD.m_fPlayerRune = READ_BYTE();
