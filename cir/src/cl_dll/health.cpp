...
/*		HealthWidth = gHUD.GetSpriteRect(gHUD.m_HUD_number_0).right - gHUD.GetSpriteRect(gHUD.m_HUD_number_0).left;
		...
		//start ice
		y = (ScreenHeight - gHUD.m_iFontHeight - gHUD.m_iFontHeight / 2) - 35;
		//end ice
		x = CrossWidth /2;
		...
		//start ice
		x = (CrossWidth + HealthWidth / 2) + 20;
		//end ice

		x = gHUD.DrawHudNumber(x, y, DHN_3DIGITS | DHN_DRAWZERO, m_iHealth, r, g, b);

		//start ice
		if ( gHUD.m_iMyFrags <= 9 )
			x = -25;
		else if ( gHUD.m_iMyFrags >= 10 && gHUD.m_iMyFrags <= 99 )
			x = -10;
		else
			x = 2;
		...
		if ( !gHUD.m_fWeaponSlotIsDown )
		{
			gHUD.DrawHudNumber(x, y, DHN_3DIGITS | DHN_DRAWZERO, gHUD.m_iMyFrags, r, g, b);
			gHUD.DrawHudNumber(x, y + 32, DHN_3DIGITS | DHN_DRAWZERO, (gHUD.m_iMyFrags/flTime) * 100, r, g, b);
		
		}

		if ( NextFPHUpdate < flTime )
		{
			char msg[64];
			sprintf(msg, "fph: %.01f\n", (gHUD.m_iMyFrags / flTime) * 100);
			ConsolePrint(msg);

			NextFPHUpdate = flTime + 2.0;
		}
		//end ice
*/
	}
	...
	if ( gHUD.m_fPlayerRune )
	{
		sprintf(szRune, "Rune: %d\n", gHUD.m_fPlayerRune);
	    gHUD.DrawHudString(x, y - 20, 320, szRune, r, g, b);
	}

	for ( int i = 1; i <= 8; i++ )
	{
		FillRGBA(x, y, gHUD.m_fPlayerHealth[i], 5, 0, 115, 230, 200);
		y = y + 10;
	}

//	DrawDamage(flTime);
	return 1;//DrawPain(flTime);
}
