...
void ClientCommand( edict_t *pEntity )
{
	...
	//start ice
	else if (FStrEq(pcmd, "givefrag" ))
	{
		GetClassPtr((CBasePlayer *)pev)->pev->frags += 1;
		GetClassPtr((CBasePlayer *)pev)->m_iDeaths += 1;

		MESSAGE_BEGIN( MSG_ALL, gmsgScoreInfo );
		WRITE_BYTE( ENTINDEX(GetClassPtr((CBasePlayer *)pev)->edict()) );
		WRITE_SHORT( GetClassPtr((CBasePlayer *)pev)->pev->frags );
		WRITE_SHORT( GetClassPtr((CBasePlayer *)pev)->m_iDeaths );
		MESSAGE_END();
/*
	TraceResult tr;
	Vector		vecSpot;
	vecSpot = pev->origin + pev->view_ofs;

	UTIL_MakeVectors(pev->angles);
	UTIL_TraceLine ( pev->origin, pev->origin + gpGlobals->v_up * -128, ignore_monsters, pev->owner, & tr);

	UTIL_DecalTrace( &tr, DECAL_RFOOT );
*/
	}
	...
	else if ( FStrEq(pcmd, "stop_spectate" ) )
	{
		// Prevent this is the cvar is set
		if ( allow_spectators.value )
		{
	...
	else if ( FStrEq(pcmd, "drop_rune" ) )
	{
		...
		if ( pPlayer->m_fHasRune )
		{
			...
			ClientPrint(pPlayer->pev, HUD_PRINTCENTER, "Discarded Current Rune\n");
		}
	}
	//end ice
...
// player pain sounds
	...
	//start ice
	PRECACHE_SOUND("player/pl_spawn1.wav");
	PRECACHE_SOUND("player/pl_jump1.wav");
	PRECACHE_SOUND("items/env_materialize1.wav");
	PRECACHE_SOUND("items/env_materialize2.wav");
	PRECACHE_SOUND("player/pl_wpickup1.wav");
	
	PRECACHE_SOUND("player/pl_headshot1.wav");
	PRECACHE_SOUND("player/pl_stomachshot1.wav");
	PRECACHE_SOUND("player/pl_breathe1.wav");
	PRECACHE_SOUND("player/pl_heartbeat1.wav");
	PRECACHE_SOUND("player/pl_bodysplat1.wav");
