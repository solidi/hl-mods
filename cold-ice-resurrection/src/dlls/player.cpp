...
void LinkUserMessages( void )
{
	...
	gmsgHealthStat = REG_USER_MSG( "HealthStat", -1 );
	gmsgRune = REG_USER_MSG( "Rune", 1 );
	//end ice
}
...
void CBasePlayer :: TraceAttack( entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int bitsDamageType)
{
	...
	flDamage = flDamage * 2; //up the damage
	...
	if ( pev->takedamage )
	{
		...
		case HITGROUP_GENERIC:
			Damage_Spot = RANDOM_LONG(HEALTH_HEAD,HEALTH_L_LEG);
			break;
		case HITGROUP_HEAD:
			flDamage *= gSkillData.plrHead;
			Damage_Spot = HEALTH_HEAD;
			pev->punchangle.x -= 5;
			EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "player/pl_headshot1.wav", 1.0, ATTN_NORM, 0, 94 + RANDOM_LONG(0,0xF));
			break;
		case HITGROUP_CHEST:
			flDamage *= gSkillData.plrChest;
			Damage_Spot = HEALTH_CHEST;
			break;
		case HITGROUP_STOMACH:
			flDamage *= gSkillData.plrStomach;
			Damage_Spot = HEALTH_STOMACH;
			pev->punchangle.x += 5;
			EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "player/pl_stomachshot1.wav", 1.0, ATTN_NORM, 0, 94 + RANDOM_LONG(0,0xF));
			break;
		case HITGROUP_LEFTARM:
			Damage_Spot = HEALTH_L_ARM;
			pev->punchangle.y += 5;
			flDamage *= gSkillData.plrArm;
			break;
		case HITGROUP_RIGHTARM:
			Damage_Spot = HEALTH_R_ARM;
			pev->punchangle.y -= 5;
			flDamage *= gSkillData.plrArm;
			break;
		case HITGROUP_LEFTLEG:
			Damage_Spot = HEALTH_L_LEG;
			pev->punchangle.x += 5;
			pev->punchangle.y += 5;
			flDamage *= gSkillData.plrLeg;
			break;
		case HITGROUP_RIGHTLEG:
			Damage_Spot = HEALTH_R_LEG;
			pev->punchangle.x += 5;
			pev->punchangle.y -= 5;
			flDamage *= gSkillData.plrLeg;
			break;
		...
		if ( pev->punchangle.x > 30 )
			pev->punchangle.x = 30;
		if ( pev->punchangle.x < -30 )
			pev->punchangle.x = -30;
		if ( pev->punchangle.y > 30 )
			pev->punchangle.y = 30;
		if ( pev->punchangle.y < -30 )
			pev->punchangle.y = -30;
		...
		if ( m_fHasRune == RUNE_PROTECT )
		{
			...
			UTIL_ScreenFade( this, Vector(200,0,0), .5, .5, 64, FFADE_IN);

		CBaseEntity *pAttacker = CBaseEntity::Instance(pevAttacker);    //base attacker
		CBasePlayer *pVictim = GetClassPtr((CBasePlayer *)pev);         //victim
		CBasePlayer *pAttack = GetClassPtr((CBasePlayer *)pevAttacker); //attacker

		//vampire rune
		if( pAttack->IsPlayer() && pAttack->m_fHasRune == RUNE_VAMPIRE && (pVictim != pAttack) )
		{
			for ( int i = 1; i <= 8; i++ )
			{
				//under limit, increase damage / 2.
				if( pAttack->m_fPartHealth[i] < 100 )
					pAttack->m_fPartHealth[i] += (flDamage / 2);
			
				//over the limit, go back to max.
				if( pAttack->m_fPartHealth[i] > 100 )
					pAttack->m_fPartHealth[i] = 100; 
			}

			pAttack->UpdateHealthStatus();
			UTIL_ScreenFade( this, Vector(200,0,0), .5, .5, 32, FFADE_IN);
		}

		//strength rune
		if ( pAttack->IsPlayer() && pAttack->m_fHasRune == RUNE_STRENGTH && (pVictim != pAttack) )
		{	
			flDamage *= 1.5; // 150%
			UTIL_ScreenFade( pAttack, Vector(200,200,0), 1, 1, 32, FFADE_IN);
		}

		//take away part health
		m_fPartHealth[Damage_Spot] -= flDamage;

		//if part is less than 0, make 0 / else take 1/4 away from shock
		if ( m_fPartHealth[Damage_Spot] < 0 )
			m_fPartHealth[Damage_Spot] = 0;
		else
			m_fPartHealth[HEALTH_SHOCK] -= (flDamage * .25);

		//if part is dead and shock is still alive, take 100% damage to shock.
		if ( m_fPartHealth[Damage_Spot] <= 0 && m_fPartHealth[HEALTH_SHOCK] > 0 )
		{
			Damage_Spot = HEALTH_SHOCK;
			m_fPartHealth[HEALTH_SHOCK] -= flDamage;
		}

		if ( m_fPartHealth[HEALTH_SHOCK] < 0 )
			m_fPartHealth[HEALTH_SHOCK] = 0;

		UpdateHealthStatus();

		//kill player if no shock left
		if ( m_fPartHealth[Damage_Spot] <= 0 )
		{
			...
			pev->health = 0;
			m_iClientHealth = 0;
			Killed(g_pevLastInflictor, 0);
		}

		if ( bitsDamageType == DMG_BULLET )
			UTIL_BloodStream( ptr->vecEndPos, vecDir, (unsigned short)73, 100 );
...
void CBasePlayer::Killed( entvars_t *pevAttacker, int iGib )
{
	...
	UTIL_ScreenFade( this, Vector(0,0,0), 6, 15, 255, FFADE_OUT | FFADE_MODULATE );
...
// many features in this function use v_forward, so makevectors now.
	...
	//start ice
	EMIT_SOUND_DYN( ENT(pev), CHAN_BODY, "player/pl_jump1.wav", 1.0, ATTN_NORM, 0, 98 + RANDOM_LONG(0,3) );
	//end ice
...
void CBasePlayer :: PlayStepSound(int step, float fvol)
{
	...
	case STEP_COLDSNOW:
		...
		// left foot
		case 2:	EMIT_SOUND( ENT(pev), CHAN_BODY, "player/pl_snow2.wav", fvol, ATTN_NORM);	break;
		case 3:	EMIT_SOUND( ENT(pev), CHAN_BODY, "player/pl_snow4.wav", fvol, ATTN_NORM);	break;
		}
		break;
	//end ice
	case STEP_VENT:
...
switch (chTextureType)
	...
	//start ice
	case CHAR_TEX_COLDSNOW: return STEP_COLDSNOW;
	//end ice
	}
}
...
void CBasePlayer::PreThink(void)
{
	...
	UTIL_TraceLine( vecSrc, vecSrc + vecAiming * 64, dont_ignore_monsters, edict(), &trace );

	if ( /*trace.pHit && Instance( trace.pHit )->pev->solid == SOLID_BSP*/ trace.flFraction < 1.0 && !FBitSet( pev->flags, FL_ONGROUND ) )
	{
		if( m_afButtonPressed == IN_JUMP && pev->velocity.Length2D() > 150 && m_fCanWallJump )
		{
			pev->velocity = -pev->velocity.Normalize() * 500 + gpGlobals->v_up * 500;
			SetAnimation( PLAYER_SUPERJUMP );
			EMIT_SOUND(ENT(pev), CHAN_VOICE, "player/pl_fallpain3.wav", 1.0, ATTN_NORM);
			m_fCanWallJump = FALSE;
		}
	}
	...
	if ( m_fShowIntro > 0 && m_fShowIntro <= gpGlobals->time )
	{
		DisplayHudMessage( "Cold-Ice Resurrection\nhttp://www.planethalflife.com/cir\n\nBy Surreal & Infinity", 2, .01, .12, 2, .05, .05, 20, .25 );
		m_fShowIntro = -1;
	}
	...
	if ( m_fHasRune == RUNE_REGEN )
	{
		if ( m_flRuneHealTime < gpGlobals->time )
		{
			for ( int i = 1; i <= 8; i++ )
			{
				if( m_fPartHealth[i] < 100 ) 
				{
					m_fPartHealth[i] += 1.0;
					UpdateHealthStatus();
					UTIL_ScreenFade( this, Vector(200,0,200), .5, .5, 32, FFADE_IN);
				}
			}
...
/*
	char sz[128];
	sprintf( sz, "yours WONID: %d\n", g_engfuncs.pfnGetPlayerWONId(edict()));
	ClientPrint(pev, HUD_PRINTCENTER, sz);
*/
	//end ice
...
// fallpain sound, and damage will be inflicted based on how far the player fell
	...
	if ( (FBitSet(pev->flags, FL_ONGROUND)) && (pev->health > 0) && m_flFallVelocity >= PLAYER_FALL_PUNCH_THRESHHOLD )
	{
		...
		{// after this point, we start doing damage
			...
			//start ice
			m_fPartHealth[HEALTH_R_LEG] -= flFallDamage;
			m_fPartHealth[HEALTH_L_LEG] -= flFallDamage;
			UpdateHealthStatus();
			//end ice
			...
			{//splat
				...
				//start ice
				EMIT_SOUND(ENT(pev), CHAN_ITEM, "player/pl_bodysplat1.wav", 1, ATTN_NORM);
				//end ice
			}
...
void CBasePlayer::Spawn( void )
{
	...
	MESSAGE_BEGIN( MSG_ONE, gmsgRune, NULL, pev );
		WRITE_BYTE( m_fHasRune );
	MESSAGE_END();
	...
	UpdateHealthStatus();
	UTIL_ScreenFade( this, Vector(0,115,230), 1, 1, 64, FFADE_IN);
	//end ice
...
// dont let uninitialized value here hurt the player
	...
	//start ice
	if ( m_fShowIntro > -1 )
		m_fShowIntro = gpGlobals->time + 5.0;

	m_fBreatheLevel = m_fBreatheTime = 0;
	m_fHeartRateLevel = m_fHeartRateTime = 0;
	//end ice
...
//start ice
void CBasePlayer::UpdateHealthStatus( void )
{
	MESSAGE_BEGIN( MSG_ONE, gmsgHealthStat, NULL, pev );
		for ( int j = 1; j <= 8; j++ )
			WRITE_BYTE( m_fPartHealth[j] );
	MESSAGE_END();
	
//	m_fHeartRateCount = 6;	
	
//	if ( !m_fHeartRateTime )
//		m_fHeartRateTime = gpGlobals->time + .402;
}
...
void CBasePlayer::Duck( )
{
	if (pev->button & IN_DUCK)
	{
		if (m_Activity != ACT_LEAP) // Don't break the LJ animation
		{
			if (m_Activity != ACT_HOP) // Don't break the LJ animation
			{
				SetAnimation( PLAYER_WALK );
			}
		}
	...
	if (pev->button & IN_DUCK)
	{
		if( (m_afButtonPressed & IN_DUCK) && !FBitSet(pev->flags, FL_DUCKING) )
		{
...
void CBasePlayer::DisplayHudMessage( char message[256], int channel, float x, float y, int effect, float fadein, float fadeout, float holdtime, float fxtime )
{
	...
	hText.effect        = effect;
	hText.r1 = hText.g1 = hText.b1 = 210;
	hText.a1 = 180;
	hText.r2 = 0;
	hText.g2 = 115;
	hText.b2 = 230;
	hText.a2 = 255;
	hText.fadeinTime    = fadeout;
	hText.fadeoutTime   = fadein;
...
void CBasePlayer::PlayerBreathe( void )
{
	//return if we are getting hurt.
	if ( m_fHeartRateLevel )
		return;

	//if im moving at a certain speed (semi-fast)
	if (pev->velocity.Length2D() > 200 )
	{
		//if im not currently in a breathe cycle
		if ( m_fBreatheTime == 0 )
			m_fBreatheTime = gpGlobals->time + 10.0;

		//if I need to breathe again, forced by my running
		if ( m_fBreatheTime && m_fBreatheTime < gpGlobals->time )
		{
			//incrase my breathing sound
			if ( m_fBreatheLevel < 1.0 )
				m_fBreatheLevel += .10;

			EMIT_SOUND(ENT(pev), CHAN_VOICE, "player/pl_breathe1.wav", m_fBreatheLevel, ATTN_NORM);
			m_fBreatheTime = gpGlobals->time + 1.406; //time based on the actual length of the sound file.
		}
	}
	else
	{
		//If im in the breathe cycle
		if ( m_fBreatheLevel > 0 )
		{
			//if its time to breathe again
			if ( m_fBreatheTime && m_fBreatheTime < gpGlobals->time )
			{
				//decrease my breathing sound
				m_fBreatheLevel -= .20;

				//im done heavy breathing
				if ( m_fBreatheLevel <= .20 )
				{
					m_fBreatheTime = 0;
					m_fBreatheLevel = 0;
				}
...
void CBasePlayer::PlayerHeartBeat( void )
{
	//start heartbeat
	if ( m_fHeartRateCount )
	{
		if ( m_fHeartRateTime && m_fHeartRateTime < gpGlobals->time )
		{
			//increase my heartbeat sound
			if ( m_fHeartRateLevel < 1.0 )
				m_fHeartRateLevel += .10;

			EMIT_SOUND(ENT(pev), CHAN_VOICE, "player/pl_heartbeat1.wav", m_fHeartRateLevel, ATTN_NORM);
			m_fHeartRateTime = gpGlobals->time + .402; //time based on the actual length of the sound file.
			m_fHeartRateCount -= 1;
		}
	}
	else
	{
		//If im in the heartbeat cycle
		if ( m_fHeartRateLevel > 0 )
		{
			//if its time to beat again
			if ( m_fHeartRateTime && m_fHeartRateTime < gpGlobals->time )
			{
				//decrease my heart rate (volume)
				m_fHeartRateLevel -= .10;

				//my heart has stopped beating fast
				if ( m_fHeartRateLevel <= .10 )
				{
					m_fHeartRateTime = 0;
					m_fHeartRateLevel = 0;
				}
