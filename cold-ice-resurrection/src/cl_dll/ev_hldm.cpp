...
float EV_HLDM_PlayTextureSound( int idx, pmtrace_t *ptr, float *vecSrc, float *vecEnd, int iBulletType )
{
	...
	case CHAR_TEX_COMPUTER:
		...
		break;
	//start ice
	case CHAR_TEX_COLDSNOW: fvol = 0.9; fvolbar = 0.2;
	...
	case CHAR_TEX_FLESH:
		...
		//start ice
		rgsz[0] = "player/pl_bulletflesh1.wav";
		rgsz[1] = "player/pl_bulletflesh1.wav";
		//end ice
		fattn = 1.0;
		cnt = 2;
		break;
...
void EV_HLDM_GunshotDecalTrace( pmtrace_t *pTrace, char *decalName )
{
	...
	if ( iRand < (0x7fff/2) )// not every bullet makes a sound.
	{
		...
		switch( iRand % 4)
		{
		case 0:	gEngfuncs.pEventAPI->EV_PlaySound( -1, pTrace->endpos, 0, "weapons/env_ric1.wav", 1.0, ATTN_NORM, 0, PITCH_NORM ); break;
		case 1:	gEngfuncs.pEventAPI->EV_PlaySound( -1, pTrace->endpos, 0, "weapons/env_ric2.wav", 1.0, ATTN_NORM, 0, PITCH_NORM ); break;
		case 2:	gEngfuncs.pEventAPI->EV_PlaySound( -1, pTrace->endpos, 0, "weapons/env_ric3.wav", 1.0, ATTN_NORM, 0, PITCH_NORM ); break;
		case 3:	gEngfuncs.pEventAPI->EV_PlaySound( -1, pTrace->endpos, 0, "weapons/env_ric4.wav", 1.0, ATTN_NORM, 0, PITCH_NORM ); break;
		}
		//end ice
	}
...
//start ice
void EV_FireSawed1( event_args_t *args )
{
	int idx;
	vec3_t origin;
	vec3_t angles;
	...
	if ( EV_IsLocal( idx ) )
	{
		// Add muzzle flash to current weapon model
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation( SAWED_FIRE_LEFT, 2 );
	}
	...
	gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/weap_sawed1.wav", gEngfuncs.pfnRandomFloat(0.95, 1.0), ATTN_NORM, 0, 93 + gEngfuncs.pfnRandomLong( 0, 0x1f ) );
...
void EV_FireSawed2( event_args_t *args )
{
	int idx;
	vec3_t origin;
	vec3_t angles;
	...
	if ( EV_IsLocal( idx ) )
	{
		// Add muzzle flash to current weapon model
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation( SAWED_FIRE_BOTH, 2 );
	}
/*
	for ( j = 0; j < 2; j++ )
	{
...
*/
	gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/weap_sawed2.wav", gEngfuncs.pfnRandomFloat(0.98, 1.0), ATTN_NORM, 0, 85 + gEngfuncs.pfnRandomLong( 0, 0x1f ) );
	...
	EV_HLDM_FireBullets( idx, forward, right, up, 16, vecSrc, vecAiming, vecSpread, 2048, BULLET_PLAYER_BUCKSHOT, 0, &tracerCount[idx-1] );
...
void EV_FireDeagle( event_args_t *args )
{
	...
	shell = gEngfuncs.pEventAPI->EV_FindModelIndex ("models/env_9mmshell.mdl");// brass shell
	...
	if ( EV_IsLocal( idx ) )
	{
		...
		// Add muzzle flash to current weapon model
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation( empty ? DEAGLE_SHOOT_EMPTY : DEAGLE_SHOOT, multiplayer ? 1 : 0 );
	}

	EV_GetDefaultShellInfo( args, origin, velocity, ShellVelocity, ShellOrigin, forward, right, up, 20, -12, 4 );

	EV_EjectBrass ( ShellOrigin, ShellVelocity * 2, angles[ YAW ], shell, TE_BOUNCE_SHELL ); 

	gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/weap_deagle1.wav", gEngfuncs.pfnRandomFloat(0.8, 0.9), ATTN_NORM, 0, PITCH_NORM );

	EV_GetGunPosition( args, vecSrc, origin );
	
	VectorCopy( forward, vecAiming );

	EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, VECTOR_CONE_1DEGREES, 8192, BULLET_PLAYER_DEAGLE, 0, &tracerCount[idx-1] );
...
void EV_FireSmg( event_args_t *args )
{
	...
	shell = gEngfuncs.pEventAPI->EV_FindModelIndex ("models/env_9mmshell_si.mdl");// brass shell
	
	if ( EV_IsLocal( idx ) )
	{
		// Add muzzle flash to current weapon model
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation( SMG_SHOOT, 2 );
	}
	...
	gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/weap_smg1.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
...
void EV_FireBlaster( event_args_t *args )
{
	int idx;
	vec3_t origin;
	vec3_t angles;
	...
	if ( EV_IsLocal( idx ) )
	{
		...
		// Add muzzle flash to current weapon model
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation( empty ? 2 : 1, multiplayer ? 1 : 0 );
	}
	...
	gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/weap_blaser1.wav", gEngfuncs.pfnRandomFloat(0.8, 0.9), ATTN_NORM, 0, PITCH_NORM );
