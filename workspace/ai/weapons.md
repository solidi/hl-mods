...
| `AcceptReload()` | — | New (see Proximity Mines, below). Default `FALSE`. Return `TRUE` to force `IN_RELOAD` to invoke `Reload()` even on `WEAPON_NOCLIP` weapons. Used by `CKnife` (zoom), `CSatchel` (prox mine), `CTripmine` (prox mine). |
...
- `ThrowGrenade()`, `ThrowSnowball()`, `ThrowRocket()` — used by the “off-hand grenade” (impulse 213) and several gamerules; see [player.md](player.md).
...
- `weapon_freezegun` → `CFreezeGun` (`freezegun.cpp`)
...
- `weapon_railgun` → `CRailgun` (`railgun.cpp`), `weapon_dual_railgun` → `CDualRailgun`
...
- `weapon_handgrenade` → `CHandGrenade` (`handgrenade.cpp`)
- `weapon_satchel` → `CSatchel` (`satchel.cpp`, slot 4/pos 2) — radio-detonated; **+reload deploys a proximity mine** (see below). Cold Ice variant: when a thrown satchel comes to rest it morphs into a `monster_human_assassin` decoy.
