...
- **Do not gate manual weapon selection on auto-switch prefs.** `cl_aws` (`m_iAutoWepSwitch`) is for auto-pickup switching (used by `FShouldSwitchWeapon`) and must not block `CBasePlayer::SelectItem`; otherwise players with `cl_aws 0` can no longer switch weapons manually (wheel, `invnext`/`invprev`, or slot binds), which often appears as a join-time "weapon switch broken" report.
