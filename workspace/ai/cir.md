...
- [weapons.md](weapons.md) — `weapon_*` entity inventory, `CBasePlayerWeapon` mechanics, proximity-mine deploy on satchel & tripmine, and explosion-seeded `napalm_pool` residue behavior. (Also linked from [client.md](client.md).)
...
- **New sound references in `src/` require a pipeline declaration update, not just code.** When adding `EMIT_SOUND`/`EMIT_SOUND_DYN`/event sound usage, verify the target file exists in `workspace/sound/` (or generated output). If the clip is missing, ask the user to provide it, then include declaration updates in the same change set: add the entry to `workspace/manifest` and add/update the corresponding `Compile-Sound` line in `workspace/Build-Sound.ps1`.
