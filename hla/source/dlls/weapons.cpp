...
void CBasePlayerWeapon::ItemPostFrame( void )
{
	...
	if ( m_pPlayer->m_fHasRune == RUNE_AMMO )
	{
		...
		{
			if ( m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType] > -1 && m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType] < iMaxAmmo1() )
				m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType]++;

			if ( m_pPlayer->m_rgAmmo[m_iSecondaryAmmoType] > -1 && m_pPlayer->m_rgAmmo[m_iSecondaryAmmoType] < iMaxAmmo2() )
				m_pPlayer->m_rgAmmo[m_iSecondaryAmmoType]++;

			if ( m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType] == iMaxAmmo1() )
			{
				if ( (m_iClip > -1) && (m_iClip < iMaxClip()) )
					m_iClip++;
			}

			 m_pPlayer->m_flAmmoRegenTime = gpGlobals->time + 1.5;
		}
	}
	//end hlpro2
...
void CBasePlayerAmmo :: DefaultTouch( CBaseEntity *pOther )
{
	...
	if (AddAmmo( pOther ))
	{
		//start hlpro2
		//char sz[128];
		//sprintf(sz, "picked up type %s ammo\n", STRING(pev->classname));
		//ALERT(at_console, sz );
		//end hlpro2
...
void CBasePlayerWeapon::DetermineArenaSpawn( CBasePlayerWeapon *pWeapon )
{
	...
	case WEAPON_CROSSBOW:	CBaseEntity::Create( "ammo_crossbow", g_pGameRules->VecWeaponRespawnSpot( pWeapon ), pWeapon->pev->angles, pWeapon->pev->owner );break;
	case WEAPON_PYTHON:		CBaseEntity::Create( "ammo_357", g_pGameRules->VecWeaponRespawnSpot( pWeapon ), pWeapon->pev->angles, pWeapon->pev->owner );break;
	case WEAPON_EGON:		CBaseEntity::Create( "ammo_egonclip", g_pGameRules->VecWeaponRespawnSpot( pWeapon ), pWeapon->pev->angles, pWeapon->pev->owner );break;
    case WEAPON_GAUSS:		CBaseEntity::Create( "ammo_gaussclip", g_pGameRules->VecWeaponRespawnSpot( pWeapon ), pWeapon->pev->angles, pWeapon->pev->owner );break;
	case WEAPON_RPG:		CBaseEntity::Create( "ammo_rpgclip", g_pGameRules->VecWeaponRespawnSpot( pWeapon ), pWeapon->pev->angles, pWeapon->pev->owner );break;
	case WEAPON_SHOTGUN:	CBaseEntity::Create( "ammo_buckshot", g_pGameRules->VecWeaponRespawnSpot( pWeapon ), pWeapon->pev->angles, pWeapon->pev->owner );break;
	case WEAPON_MP5:
		switch ( RANDOM_LONG( 0, 1 ) )
		{
		case 0:CBaseEntity::Create( "ammo_mp5clip", g_pGameRules->VecWeaponRespawnSpot( pWeapon ), pWeapon->pev->angles, pWeapon->pev->owner );break;
		case 1:CBaseEntity::Create( "ammo_ARgrenades", g_pGameRules->VecWeaponRespawnSpot( pWeapon ), pWeapon->pev->angles, pWeapon->pev->owner );break;
		}
		break;
	case WEAPON_HORNETGUN:		break;
	case WEAPON_HANDGRENADE:	CBaseEntity::Create( "weapon_handgrenade", g_pGameRules->VecWeaponRespawnSpot( pWeapon ), pWeapon->pev->angles, pWeapon->pev->owner );break;
	case WEAPON_TRIPMINE:		CBaseEntity::Create( "weapon_tripmine", g_pGameRules->VecWeaponRespawnSpot( pWeapon ), pWeapon->pev->angles, pWeapon->pev->owner );break;
	case WEAPON_SATCHEL:		CBaseEntity::Create( "weapon_satchel", g_pGameRules->VecWeaponRespawnSpot( pWeapon ), pWeapon->pev->angles, pWeapon->pev->owner );break;
	case WEAPON_SNARK:			CBaseEntity::Create( "weapon_snark", g_pGameRules->VecWeaponRespawnSpot( pWeapon ), pWeapon->pev->angles, pWeapon->pev->owner );break;
	//default:					CBaseEntity::Create( "weapon_crowbar", g_pGameRules->VecWeaponRespawnSpot( pWeapon ), pWeapon->pev->angles, pWeapon->pev->owner );
	}
}
...
void CBasePlayerAmmo::DetermineArenaAmmoSpawn( CBasePlayerAmmo *pAmmo )
{
	...
	case WEAPON_PYTHON:			CBaseEntity::Create( "ammo_357", g_pGameRules->VecAmmoRespawnSpot( pAmmo ), pAmmo->pev->angles, pAmmo->pev->owner );break;
	case WEAPON_EGON:			CBaseEntity::Create( "ammo_egonclip", g_pGameRules->VecAmmoRespawnSpot( pAmmo ), pAmmo->pev->angles, pAmmo->pev->owner );break;
	case WEAPON_GAUSS:			CBaseEntity::Create( "ammo_gaussclip", g_pGameRules->VecAmmoRespawnSpot( pAmmo ), pAmmo->pev->angles, pAmmo->pev->owner );break;
	case WEAPON_RPG:			CBaseEntity::Create( "ammo_rpgclip", g_pGameRules->VecAmmoRespawnSpot( pAmmo ), pAmmo->pev->angles, pAmmo->pev->owner );break;
	case WEAPON_SHOTGUN:		CBaseEntity::Create( "ammo_buckshot", g_pGameRules->VecAmmoRespawnSpot( pAmmo ), pAmmo->pev->angles, pAmmo->pev->owner );break;
	case WEAPON_MP5:			CBaseEntity::Create( "ammo_mp5clip", g_pGameRules->VecAmmoRespawnSpot( pAmmo ), pAmmo->pev->angles, pAmmo->pev->owner );break;
	case WEAPON_HANDGRENADE:	CBaseEntity::Create( "weapon_handgrenade", g_pGameRules->VecWeaponRespawnSpot( (CBasePlayerItem *)pAmmo ), pAmmo->pev->angles, pAmmo->pev->owner );break;
	case WEAPON_TRIPMINE:		CBaseEntity::Create( "weapon_tripmine", g_pGameRules->VecWeaponRespawnSpot( (CBasePlayerItem *)pAmmo ), pAmmo->pev->angles, pAmmo->pev->owner );break;
	case WEAPON_SATCHEL:		CBaseEntity::Create( "weapon_satchel", g_pGameRules->VecWeaponRespawnSpot( (CBasePlayerItem *)pAmmo ), pAmmo->pev->angles, pAmmo->pev->owner );break;
	case WEAPON_SNARK:			CBaseEntity::Create( "weapon_snark", g_pGameRules->VecWeaponRespawnSpot( (CBasePlayerItem *)pAmmo ), pAmmo->pev->angles, pAmmo->pev->owner );break;
	}
}
//end hlpro2
