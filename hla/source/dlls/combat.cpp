...
void RadiusDamage( Vector vecSrc, entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, float flRadius, int iClassIgnore, int bitsDamageType )
{
	...
	BOOL hit = FALSE;
	int weapon_used = 0;
	const char *killer_weapon_name = STRING(pevInflictor->classname);

	CBasePlayer *ptr = (CBasePlayer *)CBaseEntity::Instance( pevAttacker );
	...
	while ((pEntity = UTIL_FindEntityInSphere( pEntity, vecSrc, flRadius )) != NULL)
	{
		if ( pEntity->pev->takedamage != DAMAGE_NO )
		{
			...
			if ( tr.flFraction == 1.0 || tr.pHit == pEntity->edict() )
			{
				...
				ALERT(at_console, UTIL_VarArgs("pevInflictor: %s\npevAttacker: %s\nhurting: %s\n", STRING(pevInflictor->classname), STRING(pevAttacker->classname), STRING(pEntity->pev->netname)));
				//end hlpro2
...
Vector CBaseEntity::FireBulletsPlayer ( ULONG cShots, Vector vecSrc, Vector vecDirShooting, Vector vecSpread, float flDistance, int iBulletType, int iTracerFreq, int iDamage, entvars_t *pevAttacker, int shared_rand )
{
	...
	for ( ULONG iShot = 1; iShot <= cShots; iShot++ )
	{
		...
		//start hlpro2
		CBasePlayer *m_pPlayer = (CBasePlayer *)UTIL_PlayerByIndex( ENTINDEX(ENT(pevAttacker)) );
		//end hlpro2
		...
		if (tr.flFraction != 1.0)
		{
			...
			//start hlpro2
			if ( tr.pHit->v.takedamage && pEntity->IsPlayer() && pEntity->IsAlive() )
			{
				if ( iBulletType == BULLET_PLAYER_9MM )
					m_pPlayer->m_iLandedHits[WEAPON_GLOCK]++;
				else if ( iBulletType == BULLET_PLAYER_MP5 )
					m_pPlayer->m_iLandedHits[WEAPON_MP5]++;
				else if ( iBulletType == BULLET_PLAYER_BUCKSHOT )
					m_pPlayer->m_iLandedHits[WEAPON_SHOTGUN]++;
				else if ( iBulletType == BULLET_PLAYER_357 )
					m_pPlayer->m_iLandedHits[WEAPON_PYTHON]++;
			}
			else
			{
				if ( iBulletType == BULLET_PLAYER_9MM )
					m_pPlayer->m_iMissedHits[WEAPON_GLOCK]++;
				else if ( iBulletType == BULLET_PLAYER_MP5 )
					m_pPlayer->m_iMissedHits[WEAPON_MP5]++;
				else if ( iBulletType == BULLET_PLAYER_BUCKSHOT )
					m_pPlayer->m_iMissedHits[WEAPON_SHOTGUN]++;
				else if ( iBulletType == BULLET_PLAYER_357 )
					m_pPlayer->m_iMissedHits[WEAPON_PYTHON]++;
			}
			//end hlpro2
