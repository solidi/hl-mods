...
void ClientCommand( edict_t *pEntity )
{
	...
	{
		if ( GetClassPtr((CBasePlayer *)pev)->m_bWeaponStats )
		{
			GetClassPtr((CBasePlayer *)pev)->m_bWeaponStats = FALSE;
			CLIENT_PRINTF( pEntity, print_console, "Weapon status OFF.\n");
		}
		else
		{
			GetClassPtr((CBasePlayer *)pev)->m_bWeaponStats = TRUE;
			CLIENT_PRINTF( pEntity, print_console, "Weapon status ON.\n");
		}
	}
	else if ( FStrEq(pcmd, "rune" ) )
	{
		CBasePlayer *pPlayer = GetClassPtr((CBasePlayer *)pev);
	...
	else if ( FStrEq(pcmd, "drop_rune" ) )
	{
		...
		if ( pPlayer->m_fHasRune )
		{
			...
			ClientPrint(pPlayer->pev, HUD_PRINTNOTIFY, "* You have destroyed the rune.\n");
		}
		else
...
const char *GetGameDescription()
{
	if ( g_pGameRules ) // this function may be called before the world has spawned, and the game rules initialized
		...
		return "Half-Life Advanced"; //start hlpro2
}
