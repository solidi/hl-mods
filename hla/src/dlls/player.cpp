...
void CBasePlayer::PlayerDeathThink(void)
{
	...
	//start hlpro2
	if ( HasWeapons() && g_GameMode != GAME_LMS && g_GameMode != GAME_ARENA)
	{
...
int CBasePlayer::AddPlayerItem( CBasePlayerItem *pItem )
{
	...
	if (pItem->AddToPlayer( this ))
	{
		//start hlpro2
		CBasePlayerWeapon *gun;
		gun = (CBasePlayerWeapon *)pItem->GetWeaponPtr();
...
//	Vector vecSrc = plr->pev->origin + gpGlobals->v_forward * m_iObserverForwardView; //+ gpGlobals->v_up * 32;
	...
	//TraceResult tr;
	//CBaseEntity *ent = NULL;
	//while ( (ent = UTIL_FindEntityInSphere( ent, plr->pev->origin, 128 )) != NULL )
	//{
		// if ent is a client, kill em (unless they are ourselves)
		//if ( !strcmp(STRING(ent->pev->classname), "weapon_9mmAR"  ) && !ent->pev->owner)
		//{

			//UTIL_MakeVectors(plr->pev->angles);
			//UTIL_TraceLine ( plr->pev->origin, pev->origin, ignore_monsters, pev->owner, & tr);
			pev->angles = pev->v_angle = plr->pev->v_angle;
			pev->view_ofs = plr->pev->view_ofs;
