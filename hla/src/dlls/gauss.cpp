...
//	ALERT( at_console, "%f %f\n", tr.flFraction, flMaxFrac );
	...
	while (flDamage > 10 && nMaxHits > 0)
	{
		...
		if (pEntity->pev->takedamage)
		{
			...
			//start hlpro2
			if ( pEntity->IsPlayer() && (CBasePlayer *)pEntity != m_pPlayer )
			{
				m_pPlayer->m_iLandedHits[WEAPON_GAUSS]++;
				hit = TRUE;
			}
			//end hlpro2
		}
	...
	//start hlpro2
	if ( !hit )
		m_pPlayer->m_iMissedHits[WEAPON_GAUSS]++;

	m_pPlayer->DisplayAccuracy(WEAPON_GAUSS, "Gauss");
	//end hlpro2
}
