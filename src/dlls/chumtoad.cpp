...
LINK_ENTITY_TO_CLASS( monster_snark, CChumtoadGrenade );
...
void CChumtoadGrenade::Precache( void )
{
	...
	PRECACHE_SOUND("chumtoad/hunt3.wav"); //hunt sounds
    PRECACHE_SOUND("chumtoad/bite.wav"); //yummy
}
...
void CChumtoadGrenade::HuntThink( void )
{
	...
	m_flNextHunt = gpGlobals->time + 1.0;
...
BOOL CChumtoad::Deploy( )
{
	...
	if ( flRndSound <= 0.5 )
		EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "chumtoad/hunt1.wav", 1, ATTN_NORM, 0, 100);
	else 
		EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "chumtoad/hunt2.wav", 1, ATTN_NORM, 0, 100);

	m_pPlayer->m_iWeaponVolume = QUIET_GUN_VOLUME;

	return DefaultDeploy( "models/v_chub.mdl", "models/p_squeak.mdl", CHUMTOAD_UP, "squeak" );
}


void CChumtoad::Holster( )
{
	//ClientPrint(m_pPlayer->pev, HUD_PRINTTALK, "Switching Chumtoad\n");
...
void CChumtoad::PrimaryAttack()
{
	if (m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType])
	{
		...
		if (tr.fAllSolid == 0 && tr.fStartSolid == 0 && tr.flFraction > 0.25)
		{
			...
			if ( flRndSound <= 0.5 )
				EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "chumtoad/hunt1.wav", 1, ATTN_NORM, 0, 105);
			else 
...
void CChumtoad::SecondaryAttack( void )
{
